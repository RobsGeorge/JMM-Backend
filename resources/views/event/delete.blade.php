                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">حذف حدث/مناسبة</h6>
                        </div>
                    </div>

                    <div class="card shadow mb-4">
                        <form class="user" id="regForm" method="post" action="{{ route('event.destroy', $event->EventID) }}">
                            @method('DELETE')
                            @csrf
                            <div class="card-header py-3">
                                <div class="col-sm-3 mb-3 mb-sm-0">
                                                <label>هل أنت متأكد من حذف الحدث كود:  {{$event->EventId}}؟</label>
                                                <input type="submit" class="btn-google btn-user btn-block" style="background-color: brown;" id="submit-button" value="حذف"></input>
                                                <a href="{{ route('event.index') }}">رجوع</a>
                                </div>
                            </div>
                        </form>
                    </div>
